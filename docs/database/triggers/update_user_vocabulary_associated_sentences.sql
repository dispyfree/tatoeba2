...
delimiter |
CREATE TRIGGER increment_vocabulary_associated_sent_count AFTER INSERT ON user_vocabulary_sentences
FOR EACH ROW BEGIN
    UPDATE users_vocabulary SET num_assoc_sent = num_assoc_sent + 1 WHERE id = NEW.user_vocabulary_id;
END;
| 
CREATE TRIGGER decrement_vocabulary_count AFTER DELETE ON user_vocabulary_sentences
FOR EACH ROW BEGIN
